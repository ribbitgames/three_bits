...
const CONFIG = {
    ...
    autoFire: true             // Auto-fire enabled by default
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Set up auto-fire toggle
    const autoFireToggle = document.getElementById('auto-fire-toggle');
    if (autoFireToggle) {
        autoFireToggle.checked = CONFIG.autoFire;
        autoFireToggle.addEventListener('change', () => {
            CONFIG.autoFire = autoFireToggle.checked;
        });
    }
    ...
    if (fireButton) {
        // Use both mouse and touch events for better responsiveness
        const fireDown = (e) => {
            e.preventDefault();
            if (gameState === GAME_STATE.PLAYING && !CONFIG.autoFire) {
                firePlayerWeapon();
            }
        };

        fireButton.addEventListener('mousedown', fireDown);
...
function setupCamera() {
    // Using orthographic camera for 2D-like gameplay, optimized for vertical screen
...
function createPlayer() {
    ...
    // Rotate for vertical orientation (pointing up)
    ...
    noseGeometry.rotateX(-Math.PI / 2); // Point forward (up)
    ...
    player = shipGroup;
    player.position.set(0, -6, 0); // Position closer to bottom of screen
    scene.add(player);
...
function createStarfield() {
    ...
    for (let layer = 0; layer < layerCount; layer++) {
        ...
        for (let i = 0; i < starCount; i++) {
            ...
            starPositions[i3] = Math.random() * 16 - 8;     // X: -8 to 8
            starPositions[i3 + 1] = Math.random() * 20 - 10;  // Y: -10 to 10
...
function createScrollingObstacles() {
    // Create left and right wall patterns
    const wallGeometry = new THREE.BoxGeometry(2, 0.5, 0.5);
    ...
    for (let y = -6; y < 20; y += 4) {
        ...
        leftWall.position.set(-4, y, 0);
        leftWall.scrollSpeed = 0.1;
        scene.add(leftWall);
        obstacles.push(leftWall);

        // Right wall section
        const rightWall = new THREE.Mesh(wallGeometry, wallMaterial);
        rightWall.position.set(4, y, 0);
        rightWall.scrollSpeed = 0.1;
        scene.add(rightWall);
...
function onWindowResize() {
    ...
    // Update camera based on new dimensions
    const frustumSize = 10;
...
function onPointerDown(event) {
    ...
    if (gameState === GAME_STATE.PLAYING) {
        ...
        // If manual fire (auto-fire off), fire weapon on tap/click
        if (!CONFIG.autoFire) {
            firePlayerWeapon();
        }
...
function updatePlayerTarget(event) {
    ...
    // Update player target position (X for horizontal movement in vertical game)
...
function spawnEnemy() {
    ...
    enemy.position.set(
        ...
        5 + Math.random() * 2,  // Top of screen, slightly off-screen
        0
    );
...
function firePlayerWeapon() {
    ...
    if (playerPowerUps.missile) {
        ...
        missileGeometry.rotateX(-Math.PI / 2); // Point up
...
function updateGame(delta, time) {
    ...
    // Auto-fire weapon if auto-fire is enabled and pointer is down
    if (isPointerDown && CONFIG.autoFire && gameState === GAME_STATE.PLAYING) {
    ...
    if (window.starLayers) {
        window.starLayers.forEach(starLayer => {
            ...
            for (let i = 0; i < count; i++) {
                const i3 = i * 3;
                // Move star downward (for vertical scrolling)
                positions[i3 + 1] -= starLayer.scrollSpeed * delta * 60;

                // Wrap around when star goes off-screen
                if (positions[i3 + 1] < -10) {
                    positions[i3 + 1] = 10;
                    positions[i3] = Math.random() * 16 - 8; // Randomize X when wrapping
                }
            }
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        obstacle.position.y -= obstacle.scrollSpeed * delta * 60;
        ...
        if (obstacle.position.y < -10) {
            // For wall obstacles, just move them back to the top
            if (Math.abs(obstacle.position.x) === 4) {
                obstacle.position.y = 14; // Move to far top
            } else {
                // For floating obstacles, randomize position
                obstacle.position.y = 10 + Math.random() * 5;
    ...
    for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        // Simple sinusoidal movement (horizontal for vertical game)
