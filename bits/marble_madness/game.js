...
let cameraSmoothing = 0.1; // Lower for smoother, higher for more responsive
...
function init() {
    ...
    const viewSize = window.innerHeight > window.innerWidth ? 10 : 5; // Adjust view size based on orientation
...
function setupEventListeners() {
    ...
    // Touch events - direct mapping to pointer functions
...
function animate() {
    ...
    if (gameState === GAME_STATE.PLAYING) {
        ...
        updateAcidAnimation();
        updateTimer();
        updateCamera(); // Add this line to update camera position
    }
...
/**
 * Update camera to follow the marble
 */
function updateCamera() {
    if (marble && gameState === GAME_STATE.PLAYING) {
        // Create target position based on marble position
        cameraTargetPosition.copy(marble.position);

        // Maintain the same camera height/angle by keeping the y-offset constant
        cameraTargetPosition.y = 0;

        // Calculate new camera position (smoothly move toward target)
        ...
        // Keep y position constant to maintain isometric view angle
        camera.position.y = SETTINGS.cameraDistance;

        // Keep camera looking at marble
